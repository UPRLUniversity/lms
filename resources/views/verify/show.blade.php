@php
    ...
    $theme = match ($status) {
        ...
        'revoked' => ['icon' => 'shield', 'bg' => 'bg-gold/10', 'text' => 'text-gold-ink', 'ring' => 'ring-gold/20'],
        default => ['icon' => 'x', 'bg' => 'bg-ink/5', 'text' => 'text-ink/65', 'ring' => 'ring-ink/10'],
    };
@endphp
    ...
    <div class="mx-auto max-w-md px-6 py-16 text-center">
        ...
        @if ($status === 'valid')
            ...
            <dl class="mt-8 space-y-4 rounded-2xl border border-line bg-card p-6 text-left shadow-sm">
                <div>
                    <dt class="text-xs font-medium uppercase tracking-wide text-ink/65">Awarded to</dt>
                    <dd class="mt-1 font-display text-lg font-semibold text-ink">{{ $certificate->user->name }}</dd>
                </div>
                <div>
                    <dt class="text-xs font-medium uppercase tracking-wide text-ink/65">Course</dt>
                    <dd class="mt-1 text-ink">{{ $certificate->course->title }}</dd>
                </div>
                <div class="flex gap-8">
                    <div>
                        <dt class="text-xs font-medium uppercase tracking-wide text-ink/65">Issued</dt>
                        <dd class="mt-1 text-ink">{{ $certificate->issued_at->isoFormat('D MMMM YYYY') }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium uppercase tracking-wide text-ink/65">Serial</dt>
        ...
        @elseif ($status === 'revoked')
            ...
            <p class="mt-4 text-xs text-ink/65">Revoked {{ $certificate->revoked_at->isoFormat('D MMMM YYYY') }}.</p>
        @else
