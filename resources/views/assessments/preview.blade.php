...
<x-app-layout :title="'Preview — '.$assessment->title">
    <div class="mx-auto max-w-2xl">
        <a href="{{ route('assessments.edit', [$course, $assessment]) }}" class="inline-flex items-center gap-1.5 text-sm text-ink/65 hover:text-ink focus-ring rounded">
        ...
        @if ($assessment->isPooled())
            <div class="mt-6 rounded-xl border border-line bg-card p-5">
                ...
                <p class="mt-1 text-sm text-ink/65">Each student draws a random set from these rules:</p>
                <ul class="mt-3 space-y-1.5 text-sm text-ink/75">
                    @foreach ($assessment->poolRules as $rule)
                        <li class="flex items-center gap-2">
                            <x-ui.icon name="flask" class="h-4 w-4 text-ink/65" />
                ...
                @foreach ($assessment->questions as $i => $question)
                    <li class="rounded-2xl border border-line bg-card p-5 shadow-sm">
                        <div class="flex items-center justify-between">
                            <span class="text-xs font-semibold uppercase tracking-wide text-ink/65">Question {{ $i + 1 }}</span>
                            <span class="text-xs text-ink/65">{{ $question->type->shortLabel() }}</span>
                        </div>
                        ...
                        @switch($question->type)
                            ...
                            @case(QuestionType::FillBlank)
                                <input disabled placeholder="Student types an answer" class="mt-3 block w-full rounded-lg border-line bg-surface text-sm text-ink/65">
                                @break
                            @case(QuestionType::Matching)
                                <div class="mt-3 grid grid-cols-2 gap-3 text-sm">
                                    ...
                                    <ul class="space-y-1.5">
                                        @foreach (collect($question->pairs())->shuffle() as $p)<li class="rounded-lg border border-line p-2 text-ink/65">{{ $p['right'] }}</li>@endforeach
                                    </ul>
                                </div>
                                @break
                            @case(QuestionType::Essay)
                                <textarea disabled rows="4" placeholder="Student writes a response" class="mt-3 block w-full rounded-lg border-line bg-surface text-sm text-ink/65"></textarea>
                                @break
