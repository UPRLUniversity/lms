        ...
        <x-ui.card>
            <form method="POST" action="{{ route('admin.programme-parts.update', $part) }}" class="space-y-5">
                ...
                <x-ui.field name="unlock_credits" label="Credits needed to unlock the next part" type="number"
                            :value="old('unlock_credits', $part->unlock_credits)" min="0" step="1"
                            hint="Only if the bar for progressing should differ from the stated total above. Leave blank — as it normally is — and the stated total is used. Has no effect unless this programme's progression is set to sequential." />
